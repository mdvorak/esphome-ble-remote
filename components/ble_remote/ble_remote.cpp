...
void BLERemote::broadcast_(const BLERemoteCommandData &data) {
...
void BLERemote::write(uint16_t command) {
  ...
  uint32_t nonce = 0;
  do {
    if (!random_bytes(reinterpret_cast<uint8_t *>(&nonce), sizeof(nonce))) {
      ESP_LOGE(TAG, "RNG failed; dropping command 0x%04X", command);
      return;
    }
